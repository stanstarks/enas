...
def pixel_shuffler(x, scale=2, data_format="NHWC"):
  """
  For upsampling in super resolution.
  Adopted from SRGAN-tensorflow
  https://github.com/brade31919/SRGAN-tensorflow/blob/master/lib/ops.py#L85
  ...
  """
  if data_format != "NHWC":
    raise NotImplementedError("PixelShuffler does not support data_format {}".format(data_format))
  size = tf.shape(x)
  batch_size = size[0]
  h = size[1]
  w = size[2]
  c = x.get_shape()[-1] # as list?
  ...
  output = tf.concat([phase_shift(x_s, scale, shape_1, shape_2) for x_s in input_split], axis=3)

  return output


def phase_shift(x, scale, shape_1, shape_2):
  # when batch is None
